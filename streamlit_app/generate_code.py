...
def generate_standard_code(prompt, style_json, contexts, logo_file, image_refs, api_key):
    """
    Generates Marketing Assets with 'Design Mode' (Drag & Drop + Text Edit) 
    and 'Download as Image'.
    """
    genai.configure(api_key=api_key)
    ...
    **YOUR MISSION:**
    1. **CLONE THE LAYOUT:** Look at the attached 'VISUAL REFERENCES'. Your HTML structure (grid, spacing, alignment, element placement) must mimic the reference image EXACTLY.
    2. **APPLY THE BRANDING:** Use the colors, border-radii, and shadows defined in 'STYLE GUIDES'.
    3. **INJECT CONTENT:** Use the 'USER INSTRUCTION' to determine what the text should say.
    ...
    **CONTENT RULES:**
    1. **Text:** Use specific headline if provided. Otherwise, use Lorem Ipsum.
    2. **Images:** Use `src="LOGO_TOKEN"` for the main logo. Use placeholder images for others.
    3. **Container:** Ensure the outer container has `id="capture-area"` and `class="relative overflow-hidden"`.
    ...
    **OUTPUT:** Return ONLY valid HTML code with TailwindCSS classes. Do not include markdown code blocks.
    ...
    # 6. Inject 'Design Mode' & 'Download' Scripts
    # We group controls in a div that we can hide during the screenshot
    controls_script = """
    <script src="https://html2canvas.hertzen.com/dist/html2canvas.min.js"></script>
    ...
    # Append script properly
    if "</body>" in raw_html:
        final_html = raw_html.replace("</body>", f"{controls_script}</body>")
    else:
        final_html = raw_html + controls_script
